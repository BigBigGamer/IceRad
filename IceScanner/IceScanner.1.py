...
class IceScanner(tk.Tk):
    ...
    def __init__(self):
        ...
        # self.tools.grid(row = 0,column =0,sticky = 'nswe')
        # self.graphs.grid(row=1,column=0,sticky = 'nswe')
        self.loaded = False
...
class ToolSet(tk.Frame):
    ...
    def ice_detection(self):
        if not(self.controller.loaded):
            ...
            # MonthYear = self.pathNS[len(self.pathNS) - 15:-7]
            # track = self.pathNS[len(self.pathNS) - 6:-1]
            # fname = self.pathNS[len(self.pathNS) - 18:-1]
            # try:
            ...
            #     if nMap_f[i] > 0:
            #         file.write('\n%f\t'%LaNS_f[i]+'%f\t'%LoNS_f[i]+'%f\t'%LoNS_f[i]+'%f\t'%sigNS_f[i]+'%f\t'%thetaNS_f[i]+'%d\t'%nMap_f[i])                 
