...
for currentFolder in folders:
    ...
    if day is not previous_day:
        ...
        if _dataset_.size is not 0:
            ...
            _dataset_ = np.array([[],[],[],[]])
            data_ids = group.create_dataset(prev_data_id[0:11]+'_ID',dtype = 'c')    
    ...
    if has_shapefile:
        ...
        _LaNS_ = LaNS_f[bool_mask > 0]
        _LoNS_ = LoNS_f[bool_mask > 0]
        _SigNS_ = LaNS_f[bool_mask > 0]
        _thetaNS_ = thetaNS_f[bool_mask > 0]
